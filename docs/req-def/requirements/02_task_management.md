...
1. **キーワード検索（タスク名・コメント）**:
   ...
   - **Case-Insensitive & 日本語同一視 部分一致検索**: 英大文字/小文字、日本語全角/半角（NFKC正規化）、ひらがな/カタカナを区別せず部分一致で検索する（アプリケーション層で小文字化・NFKC正規化・ひらがな→カタカナ変換した検索用テキストを生成し、DBの `SEARCH_TEXT` カラムおよび trigram インデックスに対して部分一致検索を実行）。
   - 未入力またはトリム後空文字の場合はキーワード絞り込みを行わない。
2. **優先度検索**: 完全一致（高、中、低）。未指定・全選択時は絞り込みなし。
3. **締切日検索**: 指定日当日分の全タスク（`00:00:00`〜`23:59:59 JST`）、および指定日より過去の未完了タスク（ステータスが未着手/進行中）を抽出対象とする（指定日より過去の完了済みタスクおよび締切未設定（`null`）のタスクは除外）。なお、ステータス絞り込み（完了）や完了タスク表示トグルとの併用時も、過去日付の完了タスクは抽出されず、指定日当日の完了タスクのみが表示対象となる。未指定時は締切日による絞り込みなし。
